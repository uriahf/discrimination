...
-   **Cutting Through the Confusion of the Confusion Matrix:** Techniques for Choosing Cutoffs in Order to Translate Predictions into Binary Decisions: The Probability Threshold vs. the PPCR Approach ✂️

-   **ROC Curve:** What it means and why I don't like it 😤

-   **Lift (& Gains) Curve:** What is the Lift of the prediction model? 🏋️
...
# **Cutting Through the Confusion of the Confusion Matrix** ✂️
...
## PPCR (Predicted Positives Conditional Rate): {.smaller}
...
::: {.column width="50%"}
```{r}
create_precision_recall_curve(   size = 500,   probs = list(example_dat$estimated_probabilities),   reals = list(example_dat$outcome) ) |>    plotly::layout(     plot_bgcolor  = "rgba(0, 0, 0, 0)",     paper_bgcolor = "rgba(0, 0, 0, 0)",     fig_bgcolor   = "rgba(0, 0, 0, 0)"     )
```
:::
:::::
...
-   Think carefully about the problem you are trying to solve when you translate probabilities to binary predictions: \
    Treatment Harm? Use Probability Threshold. ✂️\
    Resource Constraint? Use PPCR.

-   Watch out for unusual interpretations when examining sensitivity, specificity, or the ROC curve—they all move backward in time🕺🕰

-   The AUROC is equivalent to the C-Index in the binary case. It might provide some intuition about the performance of the model but don't take the numbers too literally 👵

-   Lift Curve is useful when facing resource constraint 🏋️
